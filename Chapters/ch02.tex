...
\todo[inline]{more detials, and the resonances induced by sextupoles}
...
The lattice features 21 families of sextupoles, used for chromatic error correction and nonlinear dynamics optimization. The total count of sextupole magnets in Sirius is 280. The 21 families are, in principle, the available search space for nonlinear dynamics optimization. Some sextupoles families are located where the dispersion function vanishes. These are the achromatic, or harmonic, families, while the families located in where there is non-zero dispersion are the chromatic families. Table presents the names of the sextupoles and their types.
...
For orbit correction, SIRIUS has two types of correction systems: fast orbit correction and slow orbit correction. For the slow orbit correction system, each arc section is equipped with 8 BPMs, resulting in a total of 160 BPMs in the storage ring. Additionally, each section has 6 horizontal correctors (CH) and 7 vertical correctors (CV), both types installed in sextupole magnets. An extra vertical corrector magnet is installed per arc, leading to 120 CHs and 160 CVs in the storage ring.

The fast orbit correction system comprises 4 horizontal and 4 vertical fast correctors (FCH and FCV, respectively) per sector, totaling 80 FCH and 80 FCV (2 FCH and 2 FCV per source point). The Slow Orbit Feedback system and(SOFB) and the Fast Orbit Feedback (FOFB) system currently oversee the slow and fast orbit corrections simultaneously.
