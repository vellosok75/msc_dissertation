...
\subsubsection{Field Errors}
    ...
    % \end{equation}
    % Equation \eqref{eq:delta_disturb} is the closed-orbit distortion for a single perturbation.
    In the presence of additional dipolar and quadrupolar fields $\Delta G$ and $\Delta K$, respectively, the orbit and focusing is changed. Assuming these are small perturbations, not strong enough to kill the beam we can evluate the changes in obrit and focusing.

    The closed orbit distortion for a distribution $\Delta G(s)$ of dipolar perturbations along the ring reads
    \begin{equation}
    ...
    As for a single quadrupole error, focusing is changed. The phase advance and thus the tune changes, as well as the beta-function. As for the tune-shift as a consequence of a gradient error acting during a small extent $\Delta s$ along the ring, we have
     \begin{equation}
        \Delta \nu \approx \frac{1}{4\pi} \beta_0 \Delta K \Delta s.
     \end{equation}
     For a distribution of errors, we must sum over the ring
     \begin{equation}
    ...
    The relative beta-function deviations, known as beta-beating, reads
     \begin{equation}
...
Energy deviations affect not only the closed orbit by means of the dispersion function but also the focusing. A more/less energetic beam has higer/lower rigidity and thus is focused differently when passes through quadrupoles. This chromatic aberration effect needs to be currected, which can be attained with the insertion of geometric aberrations provided by sextupolar fields.
...
The equations of motion for off-energy particles up to the order of terms $u\delta$ leads to the additional higher-order terms
\begin{equation}
...
\end{equation}
This means there exists an energy-dependent tune-shift effect
...
We can define the \textit{linear chromaticity} in the $i=x,y$ direction as the tune variation $\Delta \nu_i$ per energy deviation $\delta$ around $\delta = 0$
\begin{equation}
    \xi_i=\dv{\nu_i}{\delta}.
\end{equation}
This uncorrected chromaticity is also called natural chromaticity.
\begin{equation}
\xi_{\text {nat }} \approx-\frac{1}{4 \pi} \oint K \beta d s
\end{equation}

To correct this chromatic effect,  we need to introduce sextupolar fields in the lattice, specifically in the dispersive regions. At dispersive regions, off-energy particles should have a deviation from the design closed orbit given by $\Delta x=\eta \delta$. Since sextupolar fields are of the form
...
and, up to dominant order, feel the quadrupolar error
$$\Delta K_{x,y}(\delta)=\pm S\eta \delta$$
The chromaticity then in a lattice with sextupoles is
\begin{equation*}
    \xi_{x,y}=\mp \frac{1}{4\pi}\oint\beta_{x,y}(K_{x,y}- S\eta)\dd s,
\end{equation*}
which depends linearly on sextupole strengths, allowing for the correction of chromaticity to specified values.
