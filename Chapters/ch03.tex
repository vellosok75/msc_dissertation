...
Figure~\ref{fig:bracketing} illustrates the RCDS bracketing. Starting from $x_0$, taken as the first guess for the minimum, $f(x_1)$ is measured. Since it is no grater than $f(x_0)+3\sigma$, the scan continues up to $x_2$, which is updated as the the minimum. $f(x_3)$ is measured and the minimum is updated to $x_3$. When measuring $f(x_4)$, condition $f(x_4) > f(x_3) + 3\sigma$ is true and the bracketing stops. The starting point, $x_0$, is checked for the condition $f(x_0)>f(x_3)+3\sigma$. If true, as in fig.~\ref{fig:bracketing}, there is no need to scan any further and the brackets will consist on $(x_0, x_3, x_4)$. If the contidion is false, the scanning continues in the negative direction until $x^*$ satisfying $f(x^*)>f(x_3)+3\sigma$ is found, giving $(x^*, x_3, x_4)$ as the bracket.
