...
To probe the beam's position along the ring, a set of four pick-up antennas placed within the vaccum chamber at several longitudinal positions are used. These are known as Beam-position-monitors (BPMs), and are sketched in the Figure. The antennas are placed in such a manner so that the electron beam deposits mirror charges when passing by then and triggers the antenna a certain voltage signal. The determination of the beam displacements depends on the differential charges induced on the antennas when the beam is not at the center, in which case the induced charges are equal. The signal of the antennas is processed to give the horizontal and vertical positions as
\begin{equation}
    x = K_x \frac{(A+D)-(B+C)}{\Sigma}, \quad y = K_y \frac{(A+B)-(C+D)}{\Sigma},
\end{equation}
where $A,B, C,D$ refers to the intensity of the induced signal over the corresponding antenna, $\Sigma=A+B+C+D$ is the sum signal, proportional to the beam's current, and $K_x$ and $K_y$ are calibration factors, which depend  on the BPM geometry  and distances between the antennas. SIRIUS has 160 BPMs distributed along the storage ring. They allow for the determination of the centroid's positions turn-by-turn, which is needed for probing of the betatron motion. Averaging the singal allows for probing the orbit.
...
When turn-by-turn motion is viewed at a fixed position $s$, it consits on the sampling of a harmonic motion. Its fundamental frequency is the tune $\nu$. Precise measurement and monitoring of the tunes is achieved by placing a stripline, which constantly excites the beam in a narrow range of frquencies. The freuqency which is capable of exiciting a resonance peak is idnetified as the tune
\todo[inline]{Need to study it}

As formula~\eqref{eq:delta_nu} reveals, changes in the quadrupoles strengths, specially at the quadrupoles at large $\beta$-function sections, allow for the control of the tunes.  Since the tune response to quadrupole strength changes is linear, a tune-response matrix can be constructed, i.e. the Jacobian of the tunes with respect to changes in quadrupoles, so that tune changes can be expressed as
\begin{equation}
    \Delta\boldsymbol{\nu} = \vb{J}_{\boldsymbol{\nu}}\Delta \vb{K},
\end{equation}
